...
locals {
  ...
  datadog_helm_config = {
    name        = "datadog"
    description = "Datadog Helm Chart"
    timeout     = "1200"
    repository  = "https://helm.datadoghq.com"
    chart       = "datadog"
    namespace   = "datadog"
    values      = local.datadog_helm_values
  }

  datadog_helm_values = [templatefile("${path.module}/datadog-values.yaml", {
...
resource "kubernetes_namespace_v1" "datadog" {
  metadata {
    name = local.datadog_helm_config["namespace"]
    labels = {
